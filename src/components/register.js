...
export const register = () => {
  ...
  buttonBack.setAttribute("id", "ButtonRegresar");
  buttonBack.textContent = "Regresar";

  /* buttonGoogle.addEventListener('click', (e) => {
  const provider = new firebase.auth.GoogloAuthProvider();
  auth.singInWhitPopup(provider)
  .then((result) => { // Si el usuario se acredita, será dirigido al muro
    onNavigate('/wall');
  })
  ...
  formRegister.addEventListener("submit", (e) => {
    ...
    //TODO: validar que los inputs tengan algo bueno para la funcion createUser
    createUser(email, password)
      .then((userCredential) => {
        // Si el usuario se acredita, será dirigido al muro
        onNavigate("/wall");
      })
      .catch((error) => {
        // si hubo un error en el registro, retorna según el caso
        const errorCode = error.code;
        if (errorCode === "auth/email-already-in-use") {
          errorText.textContent = "*El correo ya existe";
        } else if (errorCode === "auth/weak-password") {
          errorText.textContent =
            "*Contraseña débil, debe tener al menos 6 carácteres";
